...
管线第一阶段。对输入文本做敏感词匹配和分级（0 通过 / 1 记录 / 2 紧急）。level >= 2 时触发安全短路，跳过后续情绪分析和路由，直接进入危机干预。
...
### 危机路由（Crisis Route）
高风险场景的紧急干预路径。侧重点为安全保障、危机热线引导和即时情感支持。由 risk 高位、安全过滤等级触发升段、或两者叠加触发。被安全短路跳过的请求也直接命中此路由。
...
知识路由下的一次检索流程。采用统一索引（私有案例 + 公有心理学合并到一个 Chroma 向量库）+ 混合检索（稠密向量语义匹配 + BM25 关键词精确匹配）+ RRF 融合排序。检索前由 LLM 对用户查询做分类，产出 category 和 source 元数据过滤条件，缩小候选集。检索结果作为 prompt 上下文约束 LLM 输出范围，生成回复需注明知识来源。外部 API 保留为兜底占位，未接入。
...
当安全过滤结果为 blocked 或 level >= 2 时，管线跳过情绪分析和路由，直接将 route 置为 crisis、emotion 置为 distress，进入危机干预。此机制确保高危场景零延迟响应。

### TherapyChain（已标记待退役）
独立于合同管线的 LLM 对话流程，自含情绪分析、安全检查、阶段判定和回复生成。`/api/v1/chat` 端点当前仍在使用。待新干预模块验证通过后，chat 端点改接到合同管线，随后移除 TherapyChain。
